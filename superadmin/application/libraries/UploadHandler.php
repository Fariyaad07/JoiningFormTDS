...
class UploadHandler
{
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null, $directory='') {
		$CI =& get_instance();
		$directory=$CI->session->userdata('admin_session_dir');
		
		
		
		if (!file_exists(dirname(dirname($this->get_server_var('SCRIPT_FILENAME'))).'/uploads/'.$directory.'/')) {
				mkdir(dirname(dirname($this->get_server_var('SCRIPT_FILENAME'))).'/uploads/'.$directory.'/', 0777, true);
			}
		
        $this->options = array(
            'script_url' => $this->get_full_url().'/fileupload/index/',
            'upload_dir' => dirname(dirname($this->get_server_var('SCRIPT_FILENAME'))).'/uploads/'.$directory.'/',
            'upload_url' => file_upload_base_url().$directory.'/',
            ...
            'inline_file_types' => '/\.(gif|jpe?g|png|pdf|mp3|mp4|wav|doc|docx|ppt|txt)$/i',
            ...
            'accept_file_types' => '/\.(gif|jpe?g|png|pdf|mp3|mp4|wav|doc|docx|ppt|txt)$/i',
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        $file = new \stdClass();
        $file->name = strtotime("now").$this->RemoveSpecialChar($this->get_file_name($uploaded_file, $name, $size, $type, $error,
            $index, $content_range));
